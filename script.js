...
// -------------------------------------------------------
// AKUN MANAJER (tetap hardcoded, tidak perlu Firebase)
// -------------------------------------------------------
...
window.doLogin = async function () {
  ...
  try {
    ...
    // Jika tidak ada karakter @, anggap sebagai pencarian Nama
    if (!input.includes("@")) {
      if (
    ...
    localStorage.setItem("is_logged_in", "true");
    localStorage.setItem("promo_active", userData.promoActive ? "true" : "false");
    localStorage.setItem("user_discount", (userData.discount || "10").replace("%", ""));
    localStorage.setItem(
      "user_pic",
      userData.picture ||
...
window.guestLogin = function () {
  ...
  setTimeout(() => {
    ...
    const guestUid = "guest_" + Date.now() + "_" + guestId;
